"""Booking API request/response contract models (doc 12) — the shared wire contract.

These are the single definition of the Booking API's request/response shapes, consumed by both
sides: the mock server (``app.service`` / ``app.main``) implements them, and the typed client
(``api_client``) plus the agent tools construct/parse them. Three separate response models
reflect the spec's three *disjoint* status vocabularies, each with validators that reject illegal
states (e.g. ``tech_eta_minutes`` set when the booking is not ``en_route``).
"""
...
from ..domain.booking import AppointmentWindow, CustomerInfo
